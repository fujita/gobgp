...
func unmarshalCapability(a *any.Any) (bgp.ParameterCapabilityInterface, error) {
	...
	case *api.MultiProtocolCapability:
		return bgp.NewCapMultiProtocol(ToRouteFamily(a.Family.Afi, a.Family.Safi)), nil
	...
	case *api.ExtendedNexthopCapability:
		...
		for _, t := range a.Tuples {
			...
			tuples = append(tuples, bgp.NewCapExtendedNexthopTuple(ToRouteFamily(t.NlriFamily.Afi, t.NexthopFamily.Safi), nhAfi))
		}
	...
	case *api.GracefulRestartCapability:
		...
		for _, t := range a.Tuples {
			...
			tuples = append(tuples, bgp.NewCapGracefulRestartTuple(ToRouteFamily(t.Family.Afi, t.Family.Safi), forward))
		}
		var restarting bool
		if a.Flags&0x08 > 0 {
			restarting = true
	...
	case *api.AddPathCapability:
		...
		for _, t := range a.Tuples {
			tuples = append(tuples, bgp.NewCapAddPathTuple(ToRouteFamily(t.Family.Afi, t.Family.Safi), bgp.BGPAddPathMode(t.Mode)))
		}
	...
	case *api.LongLivedGracefulRestartCapability:
		...
		for _, t := range a.Tuples {
			...
			tuples = append(tuples, bgp.NewCapLongLivedGracefulRestartTuple(ToRouteFamily(t.Family.Afi, t.Family.Safi), forward, t.Time))
		}
