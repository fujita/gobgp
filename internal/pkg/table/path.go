...
func (v *Vrf) ToGlobalPath(path *Path) error {
	...
	case bgp.RF_IPv4_UC:
		...
		path.OriginInfo().nlri = bgp.NewLabeledVPNIPAddrPrefix(uint8(n.P.Bits()), n.P.Addr().String(), *bgp.NewMPLSLabelStack(v.MplsLabel), v.Rd)
	...
	case bgp.RF_IPv6_UC:
		...
		pathIdentifier := path.GetNlri().PathIdentifier()
		path.OriginInfo().nlri = bgp.NewLabeledVPNIPv6AddrPrefix(uint8(n.P.Bits()), n.P.Addr().String(), *bgp.NewMPLSLabelStack(v.MplsLabel), v.Rd)
		path.GetNlri().SetPathIdentifier(pathIdentifier)
...
func (p *Path) ToGlobal(vrf *Vrf) *Path {
	...
	case bgp.RF_IPv4_UC:
		n := nlri.(*bgp.IPAddrPrefix)
		nlri = bgp.NewLabeledVPNIPAddrPrefix(uint8(n.P.Bits()), n.P.Addr().String(), *bgp.NewMPLSLabelStack(vrf.MplsLabel), vrf.Rd)
		nlri.SetPathIdentifier(pathId)
	case bgp.RF_IPv6_UC:
		n := nlri.(*bgp.IPv6AddrPrefix)
		nlri = bgp.NewLabeledVPNIPv6AddrPrefix(uint8(n.P.Bits()), n.P.Addr().String(), *bgp.NewMPLSLabelStack(vrf.MplsLabel), vrf.Rd)
		nlri.SetPathIdentifier(pathId)
	case bgp.RF_EVPN:
		n := nlri.(*bgp.EVPNNLRI)
		switch n.RouteType {
