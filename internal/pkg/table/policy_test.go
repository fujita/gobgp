...
func TestPrefixCalcurateNoRange(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, infoMap(peer), time.Now())[0]
	// test
...
func TestPrefixCalcurateNoRangeIPv6(t *testing.T) {
	...
	path := ProcessMessage(updateMsg, map[bgp.Family]*PeerInfo{bgp.RF_IPv6_UC: peer}, time.Now())[0]
	// test
...
func TestPolicyRejectOnlyPrefixSet(t *testing.T) {
	...
	updateMsg := bgp.NewBGPUpdateMessage(nil, pathAttributes, nlri)
	path1 := ProcessMessage(updateMsg, infoMap(peer), time.Now())[0]
	...
	updateMsg = bgp.NewBGPUpdateMessage(nil, pathAttributes, nlri)
	path2 := ProcessMessage(updateMsg, infoMap(peer), time.Now())[0]
...
func TestPolicyDifferentRoutefamilyOfPathAndPolicy(t *testing.T) {
	...
	pathIPv4 := ProcessMessage(updateMsgIPv4, infoMap(peerIPv4), time.Now())[0]
	// create path ipv6
	...
	pathIPv6 := ProcessMessage(updateMsgIPv6, map[bgp.Family]*PeerInfo{bgp.RF_IPv6_UC: peerIPv6}, time.Now())[0]
	// create policy
...
func TestAsPathConditionEvaluate(t *testing.T) {
	...
	path1 := ProcessMessage(updateMsg1, infoMap(peer), time.Now())[0]
	...
	path2 := ProcessMessage(updateMsg2, infoMap(peer), time.Now())[0]
...
func TestCommunityCountConditionEvaluate(t *testing.T) {
	...
	tests := []struct {
		...
		inPath: func() *Path {
			...
			updateMsg := bgp.NewBGPUpdateMessage(nil, pathAttributes, nlri)
			UpdatePathAttrs4ByteAs(logger, updateMsg.Body.(*bgp.BGPUpdate))
			return ProcessMessage(updateMsg, infoMap(peer), time.Now())[0]
		}(),
		inCommunityCount: 0,
	}, {
