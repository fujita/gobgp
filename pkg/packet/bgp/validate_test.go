...
func bgpupdateV6() *BGPMessage {
	...
	p := []PathAttributeInterface{
		NewPathAttributeOrigin(1),
		NewPathAttributeAsPath(aspath),
		ignoreError(NewPathAttributeMpReachNLRI(RF_IPv6_UC, prefixes, netip.MustParseAddr("1023::"))),
	}
...
func Test_Validate_flowspec(t *testing.T) {
	...
	a, _ := NewPathAttributeMpReachNLRI(RF_FS_IPv4_UC, []AddrPrefixInterface{n1})
