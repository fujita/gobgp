...
func (server *BgpServer) propagate(peer *Peer, family bgp.RouteFamily, dsts []*table.Destination) {
	...
	for _, targetPeer := range server.neighborMap {
		...
		if targetPeer.fsm.marshallingOptions != nil && targetPeer.fsm.marshallingOptions.AddPath[family]&bgp.BGP_ADD_PATH_SEND > 0 {
			bestList, _, _ = dstsToPaths(targetPeer.TableID(), dsts, true)
			oldList = nil
		} else if targetPeer.isRouteServerClient() {
			bestList, oldList, _ = dstsToPaths(targetPeer.TableID(), dsts, false)
		} else {
			bestList = gBestList
			oldList = gOldList
		}
...
func (server *BgpServer) propagateUpdate(peer *Peer, pathList []*table.Path) {
	...
	for family, dsts := range families {
		server.propagate(peer, family, dsts)
	}
}
