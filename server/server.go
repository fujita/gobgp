...
type BgpServer struct {
	...
	routeServerRib *table.TableManager
...
func (s *BgpServer) Start(c *config.Global) (err error) {
	...
	s.mgmtCh <- func() {
		...
		s.routeServerRib = table.NewTableManager(rfs)
...
func (server *BgpServer) addNeighbor(c *config.Neighbor) error {
	...
	if peer.isRouteServerClient() {
		...
		if len(moded) > 0 {
			server.routeServerRib.ProcessPaths(nil, moded)
		}
	}
