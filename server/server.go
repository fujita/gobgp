...
func (server *BgpServer) propagateDsts(peer *Peer, family bgp.RouteFamily, dsts []*table.Destination) {
	var bestList, gBestList, oldList, gOldList []*table.Path
	...
	for _, target := range server.neighborMap {
		if (peer == nil && target.isRouteServerClient()) || (peer != nil && peer.isRouteServerClient() != target.isRouteServerClient() || peer == target) {
			continue
		}
		if target.isAddpathSendEnabled(family) {
			bestList, oldList, _ = dstsToPaths(target.TableID(), dsts, true)
			oldList = nil
		} else if target.isRouteServerClient() {
			bestList, oldList, _ = dstsToPaths(target.TableID(), dsts, false)
		} else {
			bestList = gBestList
			oldList = gOldList
		}
		if paths := target.processOutgoingPaths(bestList, oldList); len(paths) > 0 {
			sendFsmOutgoingMsg(target, paths, nil, false)
		}
	}
}
...
func (server *BgpServer) dropPeerAllRoutes(peer *Peer, families []bgp.RouteFamily) {
	...
	for _, rf := range families {
		...
		server.propagateDsts(peer, rf, dsts)
	}
}
...
func (server *BgpServer) propagateUpdate(peer *Peer, pathList []*table.Path) {
	...
	for family, dsts := range families {
		server.propagateDsts(peer, family, dsts)
	}
}
